...
| After upgrading `0.56.0 → 0.56.2`: `HLMonitor.set_workspace: 'workspace' is required`, three times per switch | `m:set_workspace(sel)` — the bare selector 0.56.0 required | `m:set_workspace({ workspace = sel })`. **A patch release changed the signature back to the table form.** It also stopped creating a missing workspace (`if (!ws) return 0;`), so targets are now created with `hl.focus` first and focus is restored afterwards |
...
| **R3 — The Lua API is unstable across releases** — the failure mode behind two bugs during the migration , and a **patch** release changed `set_workspace` twice over: argument shape and creation semantics | Check every new field against `src/config/lua/bindings/`; keep using the mock-`hl` effective-config harness |
