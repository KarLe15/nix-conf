...
let 
  ...
  # cfg = config.karle.waybar;
  ## TODO :: change this workspace to be generated via config / presets
  workspaces_data = {
    workspaces = [
      {id = 1; icon = "";} 
      {id = 2; icon = "";}
  ...
  wireplumber_config = {
    ...
    default_sound_manager_command = "easyeffects";
    increase_volume_command = "volumectl -M 0 -d -u up";
    lower_volume_command = "volumectl -M 0 -d -u down";
  };
  clock_data = {
...
in
{
  ...
  programs.waybar = {
    ...
    settings = 
      ...
      in 
      lib.map (bar: waybarUtils.buildBarObject bar) [
        {
          name = "ts-top";
          position = "top";
          screen = "HDMI-A-2";
